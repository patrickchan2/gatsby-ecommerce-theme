...
import * as styles from './cakesmash.module.css';
...
const ShopPage = (props) => {
  ...
  const totalItems = getProductCountByTag('cakesmash');
  const shownCount = Math.min(loadedCount, totalItems);
  const hasMore = loadedCount < totalItems;
  const data = generateMockProductData(shownCount, 'cakesmash');
  ...
  return (
    <Layout>
      <div className={styles.root}>
        ...
        <Banner
          maxWidth={'650px'}
          name={`Cake Smash Themes`}
          subtitle={
            'Look to our cake smash themes for modern takes on one-and-done dressing. From midis in bold prints to dramatic floor-sweeping styles and easy all-in-ones, our edit covers every mood.'
          }
        />
        <Container size={'large'} spacing={'min'}>
