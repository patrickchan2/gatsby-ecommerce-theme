...
const ShopPage = (props) => {
  ...
  const totalItems = getProductCountByTag('new');
  const shownCount = Math.min(loadedCount, totalItems);
  const hasMore = loadedCount < totalItems;
  const data = generateMockProductData(shownCount, 'new');
    ...
    <Layout>
      <div className={styles.root}>
        ...
        <Banner
          ...
          name={`New Themes`}
          subtitle={
            'Look to our new themes for modern takes on one-and-done dressing. From midis in bold prints to dramatic floor-sweeping styles and easy all-in-ones, our edit covers every mood.'
          }
        />
        <Container size={'large'} spacing={'min'}>
