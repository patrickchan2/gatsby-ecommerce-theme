...
import * as styles from './girl.module.css';
...
const ShopPage = (props) => {
  ...
  const totalItems = getProductCountByTag('girl');
  const shownCount = Math.min(loadedCount, totalItems);
  const hasMore = loadedCount < totalItems;
  const data = generateMockProductData(shownCount, 'girl');
    ...
    <Layout>
      <div className={styles.root}>
        ...
        <Banner
          ...
          name={`Girl Themes`}
          subtitle={
            'Look to our girl’s themes for modern takes on one-and-done dressing. From midis in bold prints to dramatic floor-sweeping styles and easy all-in-ones, our edit covers every mood.'
          }
        />
        <Container size={'large'} spacing={'min'}>
